...
func TestTransferInventoryUnitsUsesFIFOAndAudits(t *testing.T) {
	t.Setenv("ADMIN_USER", "admin")
	t.Setenv("ADMIN_PASS", "SuperSecreto123")

	db, err := initDB(filepath.Join(t.TempDir(), "inventory-transfer"), defaultPaymentMethodNames())
	if err != nil {
		t.Fatalf("initDB: %v", err)
	}
	defer db.Close()
	...
	now := time.Now().Format(time.RFC3339)
	if err := ensureBusinessLocationsForTenant(db, 1, nil, []string{"Bodega origen", "Bodega destino"}, now, "test"); err != nil {
		t.Fatalf("ensure locations: %v", err)
	}
	admin := mustLoadTestUser(t, db, "admin")
	if _, err := db.Exec(`
		INSERT INTO productos (sku, tenant_id, id, nombre, linea, location, precio_venta)
		VALUES ('TRANSFER-SKU', 1, 'TRANSFER-1', 'Producto transferible', 'Operaciones', 'Bodega origen', 10000)
	`); err != nil {
	...
	if _, err := db.Exec(`
		INSERT INTO unidades (id, tenant_id, producto_id, estado, creado_en, location)
		VALUES
			('TRANSFER-U1', 1, 'TRANSFER-SKU', 'Disponible', ?, 'Bodega origen'),
			('TRANSFER-U2', 1, 'TRANSFER-SKU', 'Disponible', ?, 'Bodega origen'),
			('TRANSFER-U3', 1, 'TRANSFER-SKU', 'Disponible', ?, 'Bodega origen'),
			('TRANSFER-U4', 1, 'TRANSFER-SKU', 'Disponible', ?, 'Bodega destino')
	`, created[0], created[1], created[2], created[2]); err != nil {
		t.Fatalf("insert units: %v", err)
	}

	result, err := transferInventoryUnits(db, admin, inventoryTransferInput{
		ProductID:      "TRANSFER-1",
		SourceLocation: "Bodega origen",
		TargetLocation: "Bodega destino",
		Quantity:       2,
		Notes:          "reposición",
	}, "api", nil)
	if err != nil {
		t.Fatalf("transferInventoryUnits: %v", err)
	}
	if result.ProductID != "TRANSFER-1" || result.ProductSKU != "TRANSFER-SKU" || len(result.UnitIDs) != 2 || result.UnitIDs[0] != "TRANSFER-U1" || result.UnitIDs[1] != "TRANSFER-U2" {
		t.Fatalf("unexpected transfer result: %+v", result)
	}
	var moved, remaining int
	if err := db.QueryRow(`SELECT COUNT(*) FROM unidades WHERE tenant_id = 1 AND producto_id = 'TRANSFER-SKU' AND location = 'Bodega destino' AND estado = 'Disponible'`).Scan(&moved); err != nil {
		t.Fatalf("count moved units: %v", err)
	}
	if err := db.QueryRow(`SELECT COUNT(*) FROM unidades WHERE tenant_id = 1 AND producto_id = 'TRANSFER-SKU' AND location = 'Bodega origen' AND estado = 'Disponible'`).Scan(&remaining); err != nil {
		t.Fatalf("count remaining units: %v", err)
	}
	if moved != 3 || remaining != 1 {
		t.Fatalf("unexpected location counts moved=%d remaining=%d", moved, remaining)
	}
	var movementCount, auditCount int
	if err := db.QueryRow(`SELECT COUNT(*) FROM movimientos WHERE tenant_id = 1 AND tipo = 'traslado'`).Scan(&movementCount); err != nil {
		t.Fatalf("count transfer movements: %v", err)
	}
	if err := db.QueryRow(`SELECT COUNT(*) FROM audit_events WHERE tenant_id = 1 AND event_type = 'inventory_transferred'`).Scan(&auditCount); err != nil {
		t.Fatalf("count transfer audits: %v", err)
	}
	if movementCount != 2 || auditCount != 1 {
		t.Fatalf("unexpected transfer logs movements=%d audits=%d", movementCount, auditCount)
	}
}

func TestLegacyInventoryConsumptionRejectsAmbiguousLocations(t *testing.T) {
	db := setupTestDB(t)
	defer db.Close()

	now := time.Now().Format(time.RFC3339)
	if _, err := db.Exec(`INSERT INTO productos (sku, tenant_id, id) VALUES ('AMBIGUOUS-SKU', 1, 'AMBIGUOUS-1')`); err != nil {
		t.Fatalf("insert product: %v", err)
	}
	if _, err := db.Exec(`
		INSERT INTO unidades (id, tenant_id, producto_id, estado, creado_en, location)
		VALUES ('AMBIGUOUS-U1', 1, 'AMBIGUOUS-SKU', 'Disponible', ?, 'A'), ('AMBIGUOUS-U2', 1, 'AMBIGUOUS-SKU', 'Disponible', ?, 'B')
	`, now, now); err != nil {
		t.Fatalf("insert units: %v", err)
	}
	tx, err := db.Begin()
	...
	_, err = selectAndMarkUnitsSold(tx, 1, "AMBIGUOUS-1", 1)
	_ = tx.Rollback()
	var reqErr requestError
	if !errors.As(err, &reqErr) || reqErr.Status != http.StatusBadRequest {
		t.Fatalf("expected ambiguous location validation error, got %v", err)
	}
}

func TestInventoryLocationSummariesMarkUnknownLocationsInactive(t *testing.T) {
	db := setupTestDB(t)
	defer db.Close()
	...
	`); err != nil {
		t.Fatalf("create business locations: %v", err)
	}
	if _, err := db.Exec(`INSERT INTO business_locations (tenant_id, name, active) VALUES (1, 'Sin ubicación', 1)`); err != nil {
		t.Fatalf("insert reserved location: %v", err)
	}
	if _, err := db.Exec(`INSERT INTO productos (sku, tenant_id, id) VALUES ('UNKNOWN-LOCATION-SKU', 1, 'UNKNOWN-LOCATION-1')`); err != nil {
		t.Fatalf("insert product: %v", err)
	}
	if _, err := db.Exec(`
		INSERT INTO unidades (id, tenant_id, producto_id, estado, creado_en, location)
		VALUES ('UNKNOWN-LOCATION-U1', 1, 'UNKNOWN-LOCATION-SKU', 'Disponible', ?, 'Legacy estante')
	`, time.Now().Format(time.RFC3339)); err != nil {
		t.Fatalf("insert unit: %v", err)
	}

	locations, err := loadInventoryLocationSummaries(db, 1, "UNKNOWN-LOCATION-SKU")
	if err != nil {
		t.Fatalf("loadInventoryLocationSummaries: %v", err)
	}
	var found *inventoryLocationSummary
	for index := range locations {
		if locations[index].Location == "Legacy estante" {
			found = &locations[index]
			break
		}
	}
	if found == nil {
		t.Fatalf("expected unknown location in summaries, got %+v", locations)
	}
	if found.Active || found.CanReceive || found.Available != 1 {
		t.Fatalf("unexpected unknown location summary: %+v", *found)
	}
	var unassigned *inventoryLocationSummary
	for index := range locations {
		if locations[index].Location == "" {
			unassigned = &locations[index]
			break
		}
	}
	if unassigned == nil || unassigned.Active || unassigned.CanReceive {
		t.Fatalf("expected reserved unassigned location to stay inactive: %+v", locations)
	}
}

func TestParseInventoryAllocationsJSON(t *testing.T) {
	allocations, err := parseInventoryAllocationsJSON(`[{"location":"A","quantity":2},{"location":"B","quantity":1}]`)
	if err != nil {
		t.Fatalf("parseInventoryAllocationsJSON: %v", err)
	}
	if len(allocations) != 2 || allocations[0].Location != "A" || allocations[0].Quantity != 2 || allocations[1].Location != "B" || allocations[1].Quantity != 1 {
		t.Fatalf("unexpected allocations: %+v", allocations)
	}
	if _, err := parseInventoryAllocationsJSON(`null`); err == nil {
		t.Fatal("expected null allocations to be rejected")
	}
}

func TestSelectAndMarkUnitsSoldWithAllocationsUsesLocationFIFO(t *testing.T) {
	db := setupTestDB(t)
	defer db.Close()
	...
	if _, err := db.Exec(`INSERT INTO productos (sku, tenant_id, id) VALUES ('ALLOC-SKU', 1, 'ALLOC-1')`); err != nil {
		t.Fatalf("insert product: %v", err)
	}
	if _, err := db.Exec(`
		INSERT INTO unidades (id, tenant_id, producto_id, estado, creado_en, location)
		VALUES ('ALLOC-A1', 1, 'ALLOC-SKU', 'Disponible', ?, 'A'),
		       ('ALLOC-A2', 1, 'ALLOC-SKU', 'Disponible', ?, 'A'),
		       ('ALLOC-B1', 1, 'ALLOC-SKU', 'Disponible', ?, 'B')
	`, created[0], created[1], created[2]); err != nil {
		t.Fatalf("insert units: %v", err)
	}
	...
	ids, err := selectAndMarkUnitsByStatusWithAllocations(tx, 1, "ALLOC-1", 3, "Vendida", []inventoryAllocationInput{
		{Location: "A", Quantity: 2},
		{Location: "B", Quantity: 1},
	})
	if err != nil {
		_ = tx.Rollback()
		t.Fatalf("selectAndMarkUnitsSoldWithAllocations: %v", err)
	}
	if err := tx.Commit(); err != nil {
		t.Fatalf("commit: %v", err)
	}
	if len(ids) != 3 || ids[0] != "ALLOC-A1" || ids[1] != "ALLOC-A2" || ids[2] != "ALLOC-B1" {
		t.Fatalf("unexpected allocation FIFO ids: %v", ids)
	}
}

func TestAPIInventoryLocationsAndTransferUseTenantScopedContract(t *testing.T) {
	db, handler, tenant, token := setupTenantWriteAPIHarness(t)
	defer db.Close()

	now := time.Now().Format(time.RFC3339)
	if err := ensureBusinessLocationsForTenant(db, tenant.ID, nil, []string{"API origen", "API destino"}, now, "test"); err != nil {
		t.Fatalf("ensure locations: %v", err)
	}
	seedTenantProductWithUnits(t, db, tenant.ID, "API-TRANSFER-1", "Producto API", 25000, 2)
	if _, err := db.Exec(`UPDATE unidades SET location = 'API origen' WHERE tenant_id = ? AND producto_id = 'API-TRANSFER-1'`, tenant.ID); err != nil {
		t.Fatalf("set source location: %v", err)
	}

	locations := performAPIJSONRequest(t, handler, http.MethodGet, "/api/inventory/locations?product_id=API-TRANSFER-1", token, nil)
	if locations.Code != http.StatusOK {
		t.Fatalf("expected locations status 200, got %d body=%s", locations.Code, locations.Body.String())
	}
	locationBody := decodeAPIResponse(t, locations)
	if locationBody["product_id"] != "API-TRANSFER-1" {
		t.Fatalf("unexpected locations response: %+v", locationBody)
	}
	locationItems, ok := locationBody["locations"].([]any)
	if !ok || len(locationItems) < 3 {
		t.Fatalf("expected source, target and unassigned location entries: %+v", locationBody)
	}

	transfer := performAPIJSONRequest(t, handler, http.MethodPost, "/api/inventory/transfer", token, map[string]any{
		"product_id":      "API-TRANSFER-1",
		"source_location": "API origen",
		"target_location": "API destino",
		"quantity":        1,
	})
	if transfer.Code != http.StatusOK {
		t.Fatalf("expected transfer status 200, got %d body=%s", transfer.Code, transfer.Body.String())
	}
	transferBody := decodeAPIResponse(t, transfer)
	if transferBody["ok"] != true || transferBody["quantity"] != float64(1) {
		t.Fatalf("unexpected transfer response: %+v", transferBody)
	}
	if tenantHeader := transfer.Header().Get("X-Stocki-Tenant-ID"); tenantHeader != strconv.Itoa(tenant.ID) {
		t.Fatalf("unexpected tenant header: %q", tenantHeader)
	}
	var targetCount int
	if err := db.QueryRow(`SELECT COUNT(*) FROM unidades WHERE tenant_id = ? AND producto_id = 'API-TRANSFER-1' AND location = 'API destino'`, tenant.ID).Scan(&targetCount); err != nil {
		t.Fatalf("count transferred unit: %v", err)
	}
	if targetCount != 1 {
		t.Fatalf("expected one transferred unit, got %d", targetCount)
	}
}
...
func TestDeleteBusinessCatalogsReassignsProductsAndScopesTenant(t *testing.T) {
	...
	if _, err := db.Exec(`
		INSERT INTO unidades (id, tenant_id, producto_id, estado, creado_en, location)
		VALUES ('CATALOG-DELETE-U1', 1, 'CATALOG-DELETE-1', 'Disponible', ?, 'Ubicación origen')
	`, now); err != nil {
		t.Fatalf("insert catalog unit: %v", err)
	}
	...
	if err := db.QueryRow(`SELECT location FROM unidades WHERE tenant_id = 1 AND id = 'CATALOG-DELETE-U1'`).Scan(&unitLocation); err != nil {
		t.Fatalf("query reassigned unit: %v", err)
	}
	if unitLocation != "Ubicación destino" {
		t.Fatalf("unexpected reassigned unit location: %q", unitLocation)
	}
...
func TestLoadInventoryUnitStatsByProductIDsAggregatesByTenant(t *testing.T) {
	db := openIsolatedPostgresTestDB(t, "inventory-unit-stats")
	execTestSchema(t, db, `
		CREATE TABLE unidades (
			id TEXT PRIMARY KEY,
			tenant_id INTEGER NOT NULL,
	...
	now := time.Now().In(appTimeLocation)
	createdFirst := now.AddDate(0, 0, -4).Format(time.RFC3339)
	createdLater := now.AddDate(0, 0, -2).Format(time.RFC3339)
	soon := now.AddDate(0, 0, 10).Format(time.RFC3339)
	far := now.AddDate(0, 0, 60).Format(time.RFC3339)
	rows := []struct {
		id, productID, status, createdAt, expiresAt string
	}{
		{"U-1", "SKU-1", "Disponible", createdFirst, soon},
		{"U-2", "SKU-1", "Disponible", createdLater, ""},
		{"U-3", "SKU-1", "Prestada", createdLater, far},
		{"U-4", "SKU-1", "Reservada", createdLater, ""},
		{"U-5", "SKU-1", "Cambio", createdLater, ""},
		{"U-6", "SKU-1", "Danada", createdLater, ""},
		{"U-7", "SKU-1", "Vendida", createdLater, ""},
		{"U-8", "SKU-2", "Disponible", createdLater, ""},
		{"U-9", "SKU-1", "Disponible", createdLater, ""},
	}
	for _, row := range rows {
		if _, err := db.Exec(`
			INSERT INTO unidades (id, tenant_id, producto_id, estado, creado_en, caducidad)
			VALUES (?, 7, ?, ?, ?, ?)
		`, row.id, row.productID, row.status, row.createdAt, row.expiresAt); err != nil {
			t.Fatalf("insert unit %s: %v", row.id, err)
		}
	}
	if _, err := db.Exec(`
		INSERT INTO unidades (id, tenant_id, producto_id, estado, creado_en, caducidad)
		VALUES ('OTHER-TENANT', 8, 'SKU-1', 'Vendida', ?, '')
	`, createdLater); err != nil {
		t.Fatalf("insert other-tenant unit: %v", err)
	}

	statsByProduct, err := loadInventoryUnitStatsByProductIDs(db, 7, []string{"SKU-1", "SKU-1", "SKU-2"})
	if err != nil {
		t.Fatalf("loadInventoryUnitStatsByProductIDs: %v", err)
	}
	stats := statsByProduct["SKU-1"]
	if stats.TotalCount != 8 || stats.AvailableCount != 3 || stats.LoanedCount != 1 || stats.ReservedCount != 1 || stats.ChangeCount != 1 || stats.DamagedCount != 1 || stats.SoldCount != 1 {
		t.Fatalf("unexpected aggregate stats: %+v", stats)
	}
	if !stats.ExpiringSoon {
		t.Fatal("expected a unit expiring within 45 days")
	}
	if stats.FirstCreatedAt != createdFirst {
		t.Fatalf("expected first creation timestamp %q, got %q", createdFirst, stats.FirstCreatedAt)
	}
	if got := statsByProduct["SKU-2"].TotalCount; got != 1 {
		t.Fatalf("expected one SKU-2 unit, got %d", got)
	}
}

func TestHandleInventoryProductUnitsEnforcesTenantOwnershipAndLoanProduct(t *testing.T) {
	db := openIsolatedPostgresTestDB(t, "inventory-unit-endpoint")
	execTestSchema(t, db, `
		CREATE TABLE productos (
			sku TEXT PRIMARY KEY,
			tenant_id INTEGER NOT NULL,
		...
		CREATE TABLE product_loan_units (
			id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
			tenant_id INTEGER NOT NULL,
	...
	now := time.Now().In(appTimeLocation).Format(time.RFC3339)
	for _, product := range []struct {
		sku, visibleID string
		ownerID        any
		tenantID       int
	}{
		{sku: "SKU-1", visibleID: "P-1", ownerID: nil, tenantID: 7},
		{sku: "SKU-PRIVATE", visibleID: "P-PRIVATE", ownerID: 99, tenantID: 7},
		{sku: "SKU-OTHER", visibleID: "P-1", ownerID: nil, tenantID: 8},
	} {
		if _, err := db.Exec(`
			INSERT INTO productos (sku, tenant_id, id, owner_user_id)
			VALUES (?, ?, ?, ?)
		`, product.sku, product.tenantID, product.visibleID, product.ownerID); err != nil {
			t.Fatalf("insert product %s: %v", product.sku, err)
		}
	}
	if _, err := db.Exec(`
		INSERT INTO unidades (id, tenant_id, producto_id, estado, creado_en, caducidad)
		VALUES ('U-1', 7, 'SKU-1', 'Disponible', ?, '')
	`, now); err != nil {
		t.Fatalf("insert public unit: %v", err)
	}
	if _, err := db.Exec(`INSERT INTO product_loans (id, tenant_id, product_id) VALUES (41, 7, 'SKU-1')`); err != nil {
		t.Fatalf("insert product loan: %v", err)
	}
	if _, err := db.Exec(`INSERT INTO product_loan_units (tenant_id, product_loan_id, unit_id) VALUES (7, 41, 'U-1')`); err != nil {
		t.Fatalf("insert loan unit: %v", err)
	}

	handler := handleInventoryProductUnits(db)
	request := func(path string, user *User) *httptest.ResponseRecorder {
		req := httptest.NewRequest(http.MethodGet, path, nil)
		req = req.WithContext(context.WithValue(req.Context(), userContextKey, user))
		resp := httptest.NewRecorder()
		handler.ServeHTTP(resp, req)
	...
	publicResponse := request("/inventario/producto/unidades?producto_id=P-1", &User{ID: 42, Role: "empleado", TenantID: 7})
	if publicResponse.Code != http.StatusOK {
		t.Fatalf("expected public product response 200, got %d body=%s", publicResponse.Code, publicResponse.Body.String())
	}
	...
	if err := json.Unmarshal(publicResponse.Body.Bytes(), &publicPayload); err != nil {
		t.Fatalf("decode public product response: %v", err)
	}
	if publicPayload.ProductID != "P-1" || publicPayload.Count != 1 || len(publicPayload.Units) != 1 {
		t.Fatalf("unexpected public product payload: %+v", publicPayload)
	}

	privateResponse := request("/inventario/producto/unidades?producto_id=P-PRIVATE", &User{ID: 42, Role: "empleado", TenantID: 7})
	if privateResponse.Code != http.StatusForbidden {
		t.Fatalf("expected private product response 403, got %d body=%s", privateResponse.Code, privateResponse.Body.String())
	}

	loanResponse := request("/inventario/producto/unidades?producto_id=P-1&product_loan_id=41", &User{ID: 42, Role: "empleado", TenantID: 7})
	if loanResponse.Code != http.StatusOK {
		t.Fatalf("expected loan response 200, got %d body=%s", loanResponse.Code, loanResponse.Body.String())
	}
	var loanPayload struct {
		LoanID int `json:"loan_id"`
		Count  int `json:"count"`
	}
	if err := json.Unmarshal(loanResponse.Body.Bytes(), &loanPayload); err != nil {
		t.Fatalf("decode loan response: %v", err)
	}
	if loanPayload.LoanID != 41 || loanPayload.Count != 1 {
		t.Fatalf("unexpected loan payload: %+v", loanPayload)
	}

	mismatchedLoanResponse := request("/inventario/producto/unidades?producto_id=P-PRIVATE&product_loan_id=41", &User{ID: 42, Role: "empleado", TenantID: 7})
	if mismatchedLoanResponse.Code != http.StatusNotFound {
		t.Fatalf("expected mismatched loan response 404, got %d body=%s", mismatchedLoanResponse.Code, mismatchedLoanResponse.Body.String())
	}
}

func TestInventoryTemplateUsesAggregatesAndLazyUnitLoading(t *testing.T) {
	content, err := os.ReadFile("templates/inventario.html")
	if err != nil {
		t.Fatalf("read inventory template: %v", err)
	}
	for _, required := range []string{
		...
		"product.unitsLoaded",
		"String(unit?.location || \"\").trim() || \"Sin ubicación\"",
		"applySaleLocally = ({ productId, quantity, allocations })",
	} {
		if !bytes.Contains(content, []byte(required)) {
			t.Fatalf("inventory template should contain optimization marker %q", required)
		}
	}
}

func TestInitPostgresDBMigratesLegacyUnitLocationBeforeCreatingIndex(t *testing.T) {
	cfg, err := loadDatabaseConfig()
	if err != nil {
		t.Fatalf("loadDatabaseConfig: %v", err)
	}

	schemaName := "test_legacy_location_bootstrap_" + strconv.FormatInt(time.Now().UnixNano(), 36)
	adminDB, err := sql.Open(postgresDriverName, cfg.DSN)
	if err != nil {
		t.Fatalf("open admin db: %v", err)
	}
	...
	legacyDB, err := sql.Open(postgresDriverName, dsn)
	if err != nil {
	...
	`)
	if _, err := legacyDB.Exec(`INSERT INTO productos (sku, tenant_id, id, linea, nombre) VALUES ('LEGACY-SKU', 1, 'LEGACY-1', 'Línea', 'Producto legacy')`); err != nil {
		_ = legacyDB.Close()
		t.Fatalf("insert legacy product: %v", err)
	}
	if _, err := legacyDB.Exec(`INSERT INTO unidades (id, tenant_id, producto_id, estado, creado_en) VALUES ('LEGACY-U1', 1, 'LEGACY-SKU', 'Disponible', ?)`, time.Now().Format(time.RFC3339)); err != nil {
		_ = legacyDB.Close()
	...
	db, err := initPostgresDB(dsn, defaultPaymentMethodNames())
	if err != nil {
		t.Fatalf("initPostgresDB should migrate legacy units before location index creation: %v", err)
	}
	defer db.Close()

	var location string
	if err := db.QueryRow(`SELECT location FROM unidades WHERE tenant_id = 1 AND id = 'LEGACY-U1'`).Scan(&location); err != nil {
		t.Fatalf("query migrated unit location: %v", err)
	}
	if location != "" {
		t.Fatalf("expected empty migrated location, got %q", location)
	}
}
...
func TestEnsureLegacyOperationalColumnsRepairsProductsFromUnits(t *testing.T) {
	...
	if _, err := db.Exec(`
		INSERT INTO productos (sku, tenant_id, nombre, linea, location)
		VALUES ('SKU-LOCATION-001', 2, 'Producto con ubicación', 'Línea', 'Bodega A')
	`); err != nil {
		t.Fatalf("insert product with location: %v", err)
	}
	if _, err := db.Exec(`
		INSERT INTO unidades (id, tenant_id, producto_id, estado, creado_en)
		VALUES
			('LEGACY-U-1', 2, 'SKU-LEGACY-001', 'Disponible', ?),
			('LEGACY-U-LOCATION-1', 2, 'SKU-LOCATION-001', 'Disponible', ?)
	...
	var unitLocation string
	if err := db.QueryRow(`SELECT location FROM unidades WHERE id = 'LEGACY-U-LOCATION-1'`).Scan(&unitLocation); err != nil {
		t.Fatalf("query migrated unit location: %v", err)
	}
	if unitLocation != "Bodega A" {
		t.Fatalf("expected migrated unit location Bodega A, got %q", unitLocation)
	}
}
...
func TestAdjustInventoryProductUpdatesStockAndRetoma(t *testing.T) {
	...
	now := time.Date(2026, 3, 22, 10, 0, 0, 0, time.UTC).Format(time.RFC3339)
	if _, err := db.Exec(`
		INSERT INTO productos (sku, id, nombre, location, precio_venta, retoma_enabled)
		VALUES ('P-002', 'P-002', 'Producto test', 'Bodega A', 20000, 0)
	`); err != nil {
		t.Fatalf("seed product: %v", err)
	}
	if _, err := db.Exec(`
	...
	var defaultLocationCount int
	if err := db.QueryRow(`SELECT COUNT(*) FROM unidades WHERE producto_id = 'P-002' AND location = 'Bodega A'`).Scan(&defaultLocationCount); err != nil {
		t.Fatalf("count units by default location: %v", err)
	}
	if defaultLocationCount != 2 {
		t.Fatalf("expected 2 newly created units in Bodega A, got %d", defaultLocationCount)
	}
